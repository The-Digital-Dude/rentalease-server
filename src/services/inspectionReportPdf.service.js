...
const drawPropertyDetailsSection = (
  ...
  // Vacant Property declaration — sits directly after the details table
...
const drawDeclarationSection = async (
  ...
  const inspectorName =
    `${technician?.firstName || ""} ${technician?.lastName || ""}`.trim() ||
    "Inspector";
  const hideInspectorIdentity = template?.jobType === "Smoke";
  // Declaration text
...
const renderTitledInlinePhotos = async (
  doc,
  heading,
  mediaItems = [],
  options = {}
  ...
  await renderInlinePhotos(doc, mediaItems, options);
};
...
const renderMinimumSafetyStandardReport = async (
  ...
  await renderSectionPhotos("property-summary", "Property Summary");
...
const formatFieldValue = (field, value) => {
  ...
  if (field.type === "multi-select" || field.type === "checkbox-group") {
    ...
    return values.map((item) => resolveOptionLabel(field, item)).join(", ");
  }
...
const shouldRenderTemplateField = (field, value, excludedFieldIds = []) => {
  if (!field || excludedFieldIds.includes(field.id)) {
    return false;
  }
...
const buildTemplateRows = (fields = [], responses = {}, excludedFieldIds = []) =>
  fields
    .filter((field) =>
      shouldRenderTemplateField(field, responses[field.id], excludedFieldIds)
    )
    .filter((field) => field.type !== "table")
    .map((field) => ({
      label: field.label,
      value: formatFieldValue(field, responses[field.id]) || "N/A",
    }));

const renderTemplateTableField = (doc, field, rows = []) => {
  if (!Array.isArray(rows) || !rows.length) {
    return;
  }

  rows.forEach((row, index) => {
    const tableRows = (field.columns || [])
      .filter((column) => shouldRenderTemplateField(column, row[column.id]))
      .map((column) => ({
        label: column.label,
        value: formatFieldValue(column, row[column.id]) || "N/A",
      }));

    if (tableRows.length) {
      drawRoomDetailTable(doc, `${field.label} #${index + 1}`, tableRows);
    }
  });
};

const renderTemplateSectionFields = (
  doc,
  section,
  responses = {},
  { excludedFieldIds = [] } = {}
) => {
  const rows = buildTemplateRows(section.fields || [], responses, excludedFieldIds);
  ...
  if (!rows.length && !tableFields.length) {
    return false;
  }
  ...
  tableFields.forEach((field) =>
    renderTemplateTableField(doc, field, responses[field.id])
  );
...
const renderSmokeOnlyReport = async (
  ...
  for (const section of template.sections || []) {
    ...
    if (section.id === "smoke-alarm-inventory") {
      const nonTableRows = buildTemplateRows(
        (section.fields || []).filter((field) => field.type !== "table"),
        sectionData
      );
      const alarmRecordsField = (section.fields || []).find(
        (field) => field.id === "alarm-records"
      );

      if (nonTableRows.length || alarmRecords.length) {
        ensurePageSpace(doc, 150);
        drawSectionHeader(doc, section.title || "Smoke Alarm Inventory");

        if (nonTableRows.length) {
          drawRoomDetailTable(doc, null, nonTableRows);
        }

        for (const [index, alarm] of alarmRecords.entries()) {
          const alarmRows = (alarmRecordsField?.columns || [])
            .filter((column) => shouldRenderTemplateField(column, alarm[column.id]))
            .map((column) => ({
              label: column.label,
              value: formatFieldValue(column, alarm[column.id]) || "N/A",
            }));

          if (alarmRows.length) {
            drawRoomDetailTable(
              doc,
              `${alarmRecordsField?.label || "Smoke Alarm"} #${index + 1}`,
              alarmRows
            );
          }

          const mediaItems = getMediaItemsForRepeatableItem(
            report,
            template,
