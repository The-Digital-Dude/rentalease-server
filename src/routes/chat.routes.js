...
router.post('/initiate', authenticateUserTypes(['agency', 'super_user', 'team_member']), chatAccessMiddleware, async (req, res) => {
  try {
    ...
    if (existingActiveSession) {
      return res.status(400).json({
        success: false,
        message: 'You already have an active chat session. Please close it before starting a new one.',
        data: {
    ...
    websocketService.broadcastChatRequest({
      ...
      sessionData: {
        ...
        initialMessage: initialMessage.substring(0, 200) + (initialMessage.length > 200 ? '...' : '')
      },
...
router.get('/sessions', authenticateUserTypes(['agency', 'super_user', 'team_member']), chatAccessMiddleware, async (req, res) => {
  try {
    const { id: userId, type: userType } = req.user;
    ...
    if (status && ['waiting', 'active', 'closed', 'transferred'].includes(status)) {
      query.status = status;
    }
    ...
    const sessionsWithUnreadCount = await Promise.all(
      sessions.map(async (session) => {
        const unreadCount = await ChatMessage.getUnreadCountForUser(
          userId,
          userType === 'agency' ? 'Agency' : userType === 'super_user' ? 'SuperUser' : 'TeamMember',
          session._id
        );
...
router.get('/session/:sessionId', authenticateUserTypes(['agency', 'super_user', 'team_member']), chatAccessMiddleware, async (req, res) => {
  try {
    const { id: userId, type: userType } = req.user;
    ...
    if (userType === 'agency') {
      hasAccess = chatSession.initiatedBy.userId._id.toString() === userId.toString();
    } else if (userType === 'super_user') {
      hasAccess = true; // SuperUsers can access all chats
    } else if (userType === 'team_member') {
      hasAccess = 
        chatSession.assignedTo.userId && chatSession.assignedTo.userId._id.toString() === userId.toString() ||
    ...
    if (messages.length > 0) {
      const userTypeEnum = userType === 'agency' ? 'Agency' : 
                          userType === 'super_user' ? 'SuperUser' : 'TeamMember';
      await ChatMessage.markAllAsReadInSession(sessionId, userId, userTypeEnum);
    }
...
router.post('/message', authenticateUserTypes(['agency', 'super_user', 'team_member']), chatAccessMiddleware, async (req, res) => {
  try {
    const { id: userId, type: userType } = req.user;
    ...
    if (userType === 'agency') {
      hasAccess = chatSession.initiatedBy.userId.toString() === userId.toString();
    } else if (userType === 'super_user' || userType === 'team_member') {
      hasAccess = 
        chatSession.assignedTo.userId && chatSession.assignedTo.userId.toString() === userId.toString() ||
    ...
    if (userType === 'agency') {
      ...
      if (chatSession.assignedTo.userId) {
        ...
        recipients.push(
          ...superUsers.map(u => ({ recipientType: 'SuperUser', recipientId: u._id })),
          ...teamMembers.map(u => ({ recipientType: 'TeamMember', recipientId: u._id }))
        );
      }
    } else {
...
router.put('/session/:sessionId/accept', authenticateUserTypes(['agency', 'super_user', 'team_member']), chatAccessMiddleware, async (req, res) => {
  try {
    const { id: userId, type: userType } = req.user;
    const { sessionId } = req.params;
    ...
    await Notification.createChatAcceptedNotification(chatSession, supportAgent.name);
...
router.put('/session/:sessionId/close', authenticateUserTypes(['agency', 'super_user', 'team_member']), chatAccessMiddleware, async (req, res) => {
  try {
    const { id: userId, type: userType } = req.user;
    const { sessionId } = req.params;
    ...
    await ChatMessage.createSystemMessage(
      chatSession._id,
      'session_closed',
      `Chat session closed by ${closedByUser.name || closedByUser.companyName}. Reason: ${reason}`
    );
...
router.get('/stats', authenticateUserTypes(['agency', 'super_user', 'team_member']), async (req, res) => {
  try {
    ...
    const [
      ...
      ChatSession.aggregate([
        { $match: { 'metadata.avgResponseTime': { $exists: true, $ne: null } } },
        { $group: { _id: null, avgResponseTime: { $avg: '$metadata.avgResponseTime' } } }
      ]),
      ChatSession.aggregate([
