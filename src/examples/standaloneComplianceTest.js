...
// Simple compliance check function using direct MongoDB queries
const checkComplianceSchedule = async () => {
  try {
    console.log("🔍 Checking compliance schedules...");
    ...
    // Get all active properties using direct MongoDB query
    ...
    for (const property of properties) {
      ...
      for (const complianceType of complianceTypes) {
        ...
        const fullAddress = `${property.address.street}, ${property.address.suburb} ${property.address.state} ${property.address.postcode}`;
    ...
    existingJobs.forEach((job) => {
      console.log(`  - ${job.jobType} job (ID: ${job.job_id})`);
    });
